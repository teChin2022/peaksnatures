...
interface SlipPaymentFieldsProps {
  /** Drives the phone-handoff poller and resets the slip when the host backs out. */
  open: boolean;
  /** Amount owed — the PromptPay QR payload and the displayed total. */
  amount: number;
  platformPayment: PlatformPayment | null;
  file: File | null;
  onFileChange: (file: File | null) => void;
  /** Optional detail rendered directly above the total, e.g. a price breakdown. */
  breakdown?: ReactNode;
}

/**
 * The "transfer this much, then show me the slip" body shared by every host
 * payment dialog: PromptPay QR or bank details, the total, the slip picker and
 * the desktop "upload from your phone" handoff.
 *
 * Extracted so a second payment flow can never drift from the first — the same
 * reason InvoicePayDialog was originally shared between Wallet and Billing.
 * Owns the preview object-URL and the polling; the parent owns the file so it
 * can gate its own submit button on it.
 */
export function SlipPaymentFields({
  open,
  amount,
  platformPayment,
  file,
    ...
    pollingRef.current = setInterval(async () => {
      try {
        ...
        if (data.uploaded && data.url) {
          ...
          onFileChange(new File([blob], data.filename || "slip.jpg", { type: blob.type }));
  ...
  }, [open, isMobile, file, sessionId, t, onFileChange]);
