...
export function ReviewFormInline({
  ...
  const [categoryRatings, setCategoryRatings] = useState<Record<string, number>>({
    environment: 0, cleanliness: 0, service: 0, value: 0,
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (data.error === "ALREADY_REVIEWED") toast.error(t("errorAlreadyReviewed"));
        else if (data.error === "BOOKING_NOT_COMPLETED") toast.error(t("errorBookingNotCompleted"));
        else toast.error(t("errorSubmit"));
        return;
      }
